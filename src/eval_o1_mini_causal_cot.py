...
class DataProcessor:
    ...
    def construct_prompt(self, row: Dict[str, Any], causal_cot: bool = False) -> str:
        """Constructs the prompt from the data fields"""
        prompt = f"""{row.get('background', '')}
        \n\n
{row.get('given_info', '')}
\n\n
{row.get('question', '')}"""
...
Step 2) Determine the query type: Identify the type of query implied by the main question. Choices include "marginal probability", "conditional probability", "explaining away effect", "backdoor ad- justment set", "average treatment effect", "collider bias", "normal counterfactual question", "average treatment effect on treated", "natural direct effect" or "natural indirect effect". Your answer should only be a term from the list above, enclosed in quotation marks.
...
Step 6) Calculate the estimand: Insert the relevant data in Step 4 into the estimand, perform basic arithmetic calculations, and derive the final answer. 
There is an identifiable answer. Answer step by step. Follow the steps above, show your reasoning, and finally output one word to answer the initial question with 'Yes' or 'No'."""
        return prompt
...
You are an expert in causal inference. The following question is not a typical commonsense query, but rather a meticulously designed question created by a professor specializing in causal inference, intended to assess the students' mastery of the course content. Reason about it and then give a "yes" or "no" final answer. Before giving the answer, show your reasoning.
    ...
    def process_single_item(self, row: Dict[str, Any], response_format: BaseModel, causal_cot: bool = False) -> Dict[str, Any]:
        """Processes a single item through both inference and parsing"""
        prompt = self.construct_prompt(row, causal_cot)
...
async def process_dataset(
    ...
    with ThreadPoolExecutor(max_workers=config.max_workers) as executor:
        futures = {
            executor.submit(processor.process_single_item, row, response_format, causal_cot): row 
            for row in dataset
        }
...
if __name__ == "__main__":
    ...
    retry_ids = load_retry_ids("missing_ids_o1-mini-causal-cot.txt")
    ...
    asyncio.run(process_dataset(
        ...
        output_path=f"results-{inference_model}-{dataset_name.split('.')[0]}-causal-cot.jsonl",
